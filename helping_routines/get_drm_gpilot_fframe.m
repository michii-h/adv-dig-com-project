...
%**************************************************************************
%   returns gain pilot frame in frequency domain for DRM Robustness Mode MODE and spectrum occupancy
%   OCCUPANCY
%   References:
...
% function call:
%------------------------
% gpilot_fframe = get_drm_gpilot_fframe(mode,occupancy);
%
% Output:
%------------------------
% gpilot_fframe: pilot frame with complex pilot data vector
%
% Input:
%------------------------
...
function gpilot_fframe = get_drm_gpilot_fframe(mode,occupancy)
...
for l = 1:symbols_per_frame
    for i = 1:length(gpilot_position(l,:))
        if (gpilot_fframe(l,gpilot_position(l,i) + dc_position) == 0)
            gpilot_fframe(l,gpilot_position(l,i) + dc_position) = gpilot_data(l,i);
        end
    end
    % boost outer pilots
